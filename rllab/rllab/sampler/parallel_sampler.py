...
def _worker_collect_one_path(G, max_path_length, curriculum=None, scope=None):
    G = _get_scoped_G(G, scope)
    path = rollout(env=G.env, agent=G.policy, max_path_length=max_path_length, curriculum=curriculum)
...
def sample_paths(
    ...
    :curriculum: if not None, sample a config from the curriculum at the beginning of each episode
    ...
    collector_function = _worker_collect_one_path

    return singleton_pool.run_collect(
        collector_function,
        threshold=max_samples,
        args=(max_path_length, curriculum, scope),
        show_prog_bar=True
    )
