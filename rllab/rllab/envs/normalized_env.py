...
class NormalizedEnv(ProxyEnv, Serializable):
    ...
    def _set_state(self, state):
        self._wrapped_env._set_state(state)

    # added
    def state_vector(self):
        return self._wrapped_env.state_vector()

    # added
    def qpos_dim(self):
        return self._wrapped_env.qpos_dim()

    # added, note that state is different from observation...
    def _get_state(self):
        self._wrapped_env._get_state()

    # added, return a random set of configuration parameters in the allowed grid
    def get_random_config(self):
        return self._wrapped_env.get_random_config()

    # added
    def get_dynamics_bounds(self):
        return self._wrapped_env.get_dynamics_bounds()
