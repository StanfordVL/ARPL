...
def train(env_ind, config_num, num_agents):
    ...
    for agent_num in range(num_agents):
        ...
        saveModel(algo.policy,
                  'policy_{}_config_{}_agent_{}'.format(dynamic_environments[env_ind], config_num, agent_num))
        ...
        if config.plot_learning_curve:
            saveModel([range(config.num_iter), avg_rewards, std_rewards],
                      'rewards_{}_config_{}_agent_{}'.format(dynamic_environments[env_ind], config_num, agent_num))
...
if __name__ == '__main__':
    ...
    processes = []
    for train_config_num in range(args.config_num_lb, args.config_num_ub):
        p = Process(target=train, args=(args.env_ind, train_config_num, args.num_agents,))
        processes.append(p)
        p.start()
        time.sleep(1)
